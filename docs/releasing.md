...
- It matches what the pre-dist workflow published, which kept the old
  hand-written installer working through the transition.
...
component**, unlike the pre-1.3.0 `ssubmit-<version>-<target>.tar.gz`. dist
cannot put the version in the archive name. Anyone who pinned the old asset URLs
needs to know this; `releases/latest/download/ssubmit-<target>.tar.gz` is the
replacement.

## Installer

`ssubmit-installer.sh` is generated by dist and attached to every release. The
documented install command is the stable latest-release URL:

```shell
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/mbhall88/ssubmit/releases/latest/download/ssubmit-installer.sh | sh
```

It installs to `$CARGO_HOME/bin` (or `~/.cargo/bin`) and edits shell profiles to
put that on `PATH`. `SSUBMIT_INSTALL_DIR` overrides the location and
`--no-modify-path` suppresses the profile edit — worth knowing for cluster
installs into a shared prefix.

This replaced the hand-written `install/install.sh`, which was deleted once
1.3.0 shipped and both installers had been verified against the real release.
Two behaviour changes came with it, since the old script defaulted to
`/usr/local/bin` and never touched `PATH`:

- the default install location moved to `~/.cargo/bin`
- `--bin-dir` became the `SSUBMIT_INSTALL_DIR` environment variable

The old `raw.githubusercontent.com/.../install/install.sh` URL now 404s. See
issue #18.
